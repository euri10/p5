...
def begin_shape(kind=TESS):
    ...
    :param kind: TESS, POINTS, LINES, TRIANGLES, TRIANGLE_FAN, TRIANGLE_STRIP, QUADS,
    or QUAD_STRIP; defaults to TESS
    :type kind: SType
    """
    global shape_kind, vertices, contour_vertices, vertices_types, contour_vertices_types, is_contour  # noqa: E501
...
def end_shape(mode=""):
    ...
    if is_curve:
        ...
        shape = PShape(
            ...
            contours=[
                get_quadratic_vertices(contour_vertices[i], contour_vertices_types[i])
                for i in range(len(contour_vertices))
            ],
            shape_type=TESS,
        )
